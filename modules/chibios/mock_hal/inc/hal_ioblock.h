...
#else
bool blkIsWriteProtected(void *ip);
#endif
...
#else
bool blkRead(void *ip, uint32_t startblk, uint8_t * buf, uint32_t n);
#endif
...
#else
bool blkWrite(void *ip, uint32_t startblk, const uint8_t *buf, uint32_t n);
#endif
...
#else
bool blkGetInfo(void *ip, BlockDeviceInfo *bdip);
#endif
